	...
	\chapter{Operational Systems}
		...
		\section{Exercises}
			\begin{exercise}{Self-Assessment}
				\begin{enumerate}
					\item ERP stands for \concept{Enterprise Resource Planning}.
					\item Systems that allow an organisation to conduct business are \concept{Transaction Processing Systems}.
					\item OLTP is \concept{Online Transaction Processing}.
					\item RFID can be used to \concept{input (or collect)} the data necessary for transaction processing.
					\item The process of performing calculations and other data transformations is called \concept{data manipulation}.
					\item Buying a book on a smartphone is an example of \concept{m-commerce}.
					\item Systems used to improve interaction between a government and its citizens form \concept{e-government}.
					\item CRM stands for \concept{Customer Relationship Management}.
					\item A wholesaler selling to a high street shop via the Internet is an example of \concept{B2B commerce}.
					\item When you sell an old textbook on Amazon to another student, you are taking part in \concept{C2C} commerce.
				\end{enumerate}
			\end{exercise}
	\vbox{\rulechapterend}
\end{document}