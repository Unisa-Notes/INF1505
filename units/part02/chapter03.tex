	...
	\chaptermark{Hardware}
		...
		\section{Exercises}
			\begin{exercise}{Self-Assessment}
				\begin{enumerate}
					\item The \concept{Arithmetic Logic Unit} hardware component performs mathematical calculations.
					\item A \concept{register} holds small units of program instructions and data immediately before, during, and after the execution by the computer.
					\item \concept{MIPS} is a measure of machine cycle time.
					\item A terabyte is 2 to the power of \concept{40} bytes.
					\item Magnetic Tape is an example of \concept{secondary storage}.
					\item A \concept{thin client} is a low-cost computer with no extra drives or expansion slots.
					\item Project Gutenberg is a source of \concept{e-books}.
					\item The ability to increase processing in order to handle more users is called \concept{scalability}.
					\item Google uses \concept{seawater} to cool its servers in Finland.
					\item EPEAT assists with the assessment of \concept{green computing}.
				\end{enumerate}
			\end{exercise}
	\vbox{\rulechapterend}
\end{document}