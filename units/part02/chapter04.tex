	...
	\chapter[Software]{Software: Systems and Application Software}
		...
		\section{Exercises}	
			\begin{exercise}{Self-Assessment}
				\begin{enumerate}
					\item Software can broadly be separated into \concept{systems} and \concept{application} software.
					\item Linus was developed in 1991 by \concept{Linus Torvalds}.
					\item The PC operating system by Google is called \concept{Chrome}.
					\item An operating system designed for a device such as a smartphone is often known as \concept{embedded}.
					\item One-of-a-kind software developed for a specific business application is called \concept{proprietary software}.
					\item Providing application software via a web browser is known as \concept{software as a service}.
					\item PowerPoint is an example of \concept{graphics presentation} software.
					\item A \concept{compiler} converts source code into machine-language instructions.
					\item \concept{Open-source software} is available for free and with its source code, which can be modified.
					\item MySQL is an example of \concept{database software}.
				\end{enumerate}
			\end{exercise}
	\vbox{\rulechapterend}
\end{document}