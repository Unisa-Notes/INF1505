	...
	\chapter{Computer Networks}
		...
		\section{Exercises}
			\begin{exercise}{Self-Assessment}
				\begin{enumerate}
					\item \concept{Channel bandwidth} is the rate at which data is exchanged.
					\item Fibre-optic cable transmits data as a \concept{beam of light}.
					\item Using electricity cables for data transmission is \concept{broadband over power lines}.
					\item A very short range wireless data transmission is \concept{Near Field Communication (NFC)}.
					\item A \concept{switch} uses a physical device address in each incoming message on a network to determine to which output port it should forward the message to reach another device on the same network.
					\item \concept{Network operating system} and \concept{network management software} are communications software.
					\item The \concept{\texttt{<b>}} tag turns text on a webpage bold.
					\item \concept{Web services} allow websites to communicate with each other.
					\item FTP is \concept{File Transfer Protocol}.
					\item Google Docs and Zoho are examples of \concept{cloud computing}.
				\end{enumerate}
			\end{exercise}
